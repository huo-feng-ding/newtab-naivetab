...
export const currKeyboardConfig = computed(() => {
  ...
  try {
    ...
    if (!target.isMacOS && isMacOS) {
      ...
      for (
        let index = 0, len = target.list[lastRowIndex].length;
        index < len;
        index++
      ) {
      ...
      if (leftAltIndex !== -1 && leftMetaIndex !== -1) {
        ;[
          target.list[lastRowIndex][leftMetaIndex],
          target.list[lastRowIndex][leftAltIndex],
        ] = [
          target.list[lastRowIndex][leftAltIndex],
          target.list[lastRowIndex][leftMetaIndex],
        ]
      ...
      if (rightAltIndex !== -1 && rightMetaIndex !== -1) {
        ;[
          target.list[lastRowIndex][rightMetaIndex],
          target.list[lastRowIndex][rightAltIndex],
        ] = [
          target.list[lastRowIndex][rightAltIndex],
          target.list[lastRowIndex][rightMetaIndex],
        ]
    ...
    const spaceConfig =
      SPLIT_SPACES_CONFIG_MAP[localConfig.keyboardCommon.splitSpace]
    if (
      spaceConfig &&
      localConfig.keyboardCommon.splitSpace !== 'space1' &&
      spaceIndex !== -1
    ) {
...
export const keyboardCurrentModelAllKeyList = computed(() =>
  currKeyboardConfig.value.list.flat(Infinity),
)
